...
This tutorial follows the creation and deployment of virtual machines, observing network traffic within the public cloud computing platform Microsoft Azure..<br />
...
After filtering for ICMP traffic only, I entered the private IP Address for the Linux VM using the ping command along with the Linux private IP address from inside Windows PowerShell. We can now observe the successful ping requests and replies within Wireshark. 
...
Observing the rule that was set in Azure, succesfully disabled all incoming ICMP traffic, which begins to override the perpetual ping. After reenabling the ICMP traffic, the perpetual ping starts up again until prompting the ping activity to stop in the PowerShell with Ctrl + C. 
...
Continuing to observe network traffic by filtering for DNS traffic only. After limiting traffic shown through the packet capture, I entered the nslookup command into PowerShell to ping a public website. 
